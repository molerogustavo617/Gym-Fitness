...
const Maquinas = () => {
  ...
  const getEstadoColor = (estado) => {
    switch (estado) {
      ...
      case 'En reparación': return 'text-purple-400';
  ...
  if (loading && maquinas.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px] gap-3">
        ...
        <div className="text-gym-neon font-semibold text-sm tracking-wide">Cargando máquinas...</div>
      </div>
    );
  }
    ...
    <div className="w-full space-y-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-gym-dark-secondary p-4 md:p-5 rounded-2xl border border-gym-gray/5">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl md:text-2xl font-bold text-gym-white tracking-tight">Máquinas</h1>
            <p className="text-gym-gray text-sm hidden sm:block">Gestiona el inventario de equipos</p>
          </div>
        </div>
        <button
      ...
      <div className="relative">
        ...
        <input
          type="text"
          placeholder="Buscar máquina por nombre, modelo o serial..."
          value={searchTerm}
          onChange={(e) => {
          ...
          <table className="w-full text-sm min-w-[900px]">
            <thead>
              <tr className="border-b border-gym-gray/10 bg-gym-card/20">
                ...
                <th className="text-left py-3 px-3 text-gym-gray font-semibold text-xs uppercase tracking-wider whitespace-nowrap">Modelo</th>
                <th className="text-left py-3 px-3 text-gym-gray font-semibold text-xs uppercase tracking-wider whitespace-nowrap">Serial</th>
                ...
                <th className="text-left py-3 px-3 text-gym-gray font-semibold text-xs uppercase tracking-wider whitespace-nowrap">Adquisición</th>
              ...
              {currentItems.length === 0 ? (
                ...
                currentItems.map((maquina) => (
                  <tr key={maquina.idmaquina} className="border-b border-gym-gray/5 hover:bg-gym-card/30 transition-colors">
                    ...
                    {/* Serial */}
                    <td className="py-2.5 px-3">
                      <p className="text-gym-gray-light font-mono text-xs whitespace-nowrap">
                        {maquina.serial || '-'}
                      </p>
                    </td>
                    {/* Estado */}
                    <td className="py-2.5 px-3">
                      <span className={`font-medium ${getEstadoColor(maquina.estado)}`}>
        ...
        {filteredMaquinas.length > 0 && (
          <div className="flex flex-col sm:flex-row items-center justify-between gap-2 px-4 py-3 border-t border-gym-gray/10">
            <span className="text-gym-gray text-sm">
              {filteredMaquinas.length > 0 ? indexOfFirstItem + 1 : 0}-
              {Math.min(indexOfLastItem, filteredMaquinas.length)} de {filteredMaquinas.length}
            </span>
            <div className="flex items-center gap-1">
              <button
            ...
            <div className="p-5 md:p-6">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="text-gym-gray-light text-sm font-medium block mb-1.5">Modelo</label>
                    <input
                      name="modelo"
                      type="text"
                  ...
                  <div>
                    <label className="text-gym-gray-light text-sm font-medium block mb-1.5">Serial</label>
                    <input
                      name="serial"
                      type="text"
                      ...
                      onChange={handleInputChange}
                      className="w-full px-4 py-2.5 bg-gym-dark rounded-lg border border-gym-gray/20 text-gym-white font-mono text-sm focus:outline-none focus:border-gym-neon/50 transition-colors"
                    />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="text-gym-gray-light text-sm font-medium block mb-1.5">Estado</label>
                    <select
                      name="estado"
                  ...
                  <div>
                    <label className="text-gym-gray-light text-sm font-medium block mb-1.5">Adquisición</label>
                    <input
